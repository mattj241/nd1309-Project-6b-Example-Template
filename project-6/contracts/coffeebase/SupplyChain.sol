...
contract SupplyChain is RetailerRole, FarmerRole, ConsumerRole, DistributorRole {
  ...
  function harvestItem(
    uint _upc, address _originFarmerID, string memory _originFarmName, 
    string memory _originFarmInformation, string  memory _originFarmLatitude, 
    string  memory _originFarmLongitude, string  memory _productNotes)
    onlyFarmer public 
  {
  ...
  function packItem(uint _upc) processed(_upc)
   verifyCaller(items[_upc].originFarmerID) 
   onlyFarmer 
   public   
  {
  ...
  function buyItem(uint _upc) 
    forSale(_upc) paidEnough(msg.value) 
    checkValue(_upc) 
    onlyDistributor 
    public 
    payable  
